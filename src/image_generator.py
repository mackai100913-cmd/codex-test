...
BRAND_WORLD = (
    "【世界観】暗い木目のテーブル＋真っ黒〜濃い焦げ茶の背景。余計な小物・窓・他の食器は写さない。"
    "上後方からの暖色のスポットライトで料理だけを照らし、背景は暗く落とす。ふんわり立ち上る湯気。"
    "【画質】一眼レフ＋単焦点レンズで撮った実写。8K級の高解像度、料理にシャープなピント、"
    "背景は大きくぼかす（浅い被写界深度）。本物の家庭料理の質感。CG・イラスト・作り物に見せない。"
    "【厳禁】写真の中に文字・ロゴ・透かし・ウォーターマーク・キラキラ記号(✦等)・人物・手を一切入れない。"
)

_APPETIZING = (
    "とにかく食欲をそそる、思わず生唾を飲むほど美味しそうに。"
    "できたての熱々感、立ち上る湯気、油や煮汁の照り・テカリ、"
...
def brand_brief() -> str:
    ...
    return (
        ...
        f"{BRAND_WORLD}\n"
        "【完成料理（メイン写真）】浅く平たい黒マットの和食器に美しく盛り付ける。"
        "フライパン・スキレット・鍋・カトラリーは写さない。料理は皿いっぱいに照り良く立体的に盛り、"
        "皿のフチまで入れて画面の中央〜やや下に大きく配置。やや斜め45度から。縦長9:16。\n"
        "【調理工程】鍋やフライパンの中身を画面中央に大きく、真上または斜め上から。正方形1:1。\n"
...
def hero_prompt(recipe: Recipe) -> str:
    ...
    return (
        ...
        "【器】浅く平たい黒マットの丸い和食器に盛り付ける。"
        "フライパン・スキレット・鍋・スプーン・フォークは絶対に写さない（完成した皿のみ）。"
        "【盛り付け】具材を皿いっぱいに、照りとツヤを強調して立体的に山高く盛る。"
        "【構図】料理を画面のど真ん中〜やや下に大きく配置し、皿のフチまで写す。やや斜め45度から撮影。"
        f"{_APPETIZING}"
        f"{BRAND_WORLD}"
        "縦長（9:16）。"
    )
...
def step_prompt(recipe: Recipe, step_title: str, detail: str) -> str:
    return (
        ...
        "【構図】調理中の鍋・フライパン・まな板の中身を画面のど真ん中に大きく配置。"
        "真上からの俯瞰、または斜め上から、今まさに調理している臨場感を出す。"
        f"{_APPETIZING}"
        f"{BRAND_WORLD}"
        "メイン写真と同じ世界観・色味・ライティングで統一。正方形（1:1）。"
    )
